<template>
    <div class="relative w-full h-full flex items-center justify-center perspective-1000">
        ...
        <div 
            class="transition-transform duration-700 ease-out z-10 w-full"
            :style="{ transform: `translateY(${offsetY}px) scale(1.15)`, transformOrigin: 'bottom right' }"
        ...
        >
            ...
            <img 
                ...
                class="w-full h-auto object-contain drop-shadow-[0_20px_20px_rgba(0,155,68,0.25)]"
                @load="imageLoaded = true"
            />
            
            <!-- Loading skeleton just in case -->
...
// Placeholder image (will show a generic truck silhouette if real image not available, 
// but for now we expect the user to have their truck image)
const currentTruckImage = ref('/img/slider/truck-slide3.png'); // Change this to actual URL if needed
...
const animateFloat = (timestamp) => {
    ...
    // Sine wave for floating: 10px amplitude, 4000ms period
    if (!hovering.value) {
        offsetY.value = Math.sin(progress / 600) * 8;
    } else {
        // When hovering, float slightly higher and stop bobbing
        offsetY.value = -15;
    }
