@extends('layouts.base', ['title' => 'Sales Profile - ' . $sale->name])
    ...
    <div class="relative bg-gradient-to-r from-gray-900 to-green-900 pb-32 pt-20">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
            ...
            <p class="mt-6 text-xl text-gray-300 max-w-3xl mx-auto italic font-medium">
                "{{ $sale->slogan ?? $sale->tagline ?? 'Melayani dengan sepenuh hati dan profesionalisme.' }}"
            </p>
        </div>
    </div>
    ...
    <div class="-mt-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20 mb-16">
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col md:flex-row">
            <div class="md:w-1/3 bg-gray-50 p-8 flex flex-col items-center justify-center border-r border-gray-100">
                @if($sale->photo)
                    <img src="{{ asset('storage/'.$sale->photo) }}" alt="{{ $sale->name }}" class="w-48 h-48 rounded-full object-cover border-4 border-green-500 shadow-lg mb-6">
                @else
                ...
                @endif
                <h2 class="text-2xl font-bold text-gray-900">{{ $sale->name }}</h2>
                <p class="text-green-600 font-semibold mb-2">Official HINO Sales Executive</p>
                @if($sale->specialties)
                    <p class="text-sm font-bold bg-green-100 text-green-800 px-3 py-1 rounded-full mb-4">{{ $sale->specialties }}</p>
                @endif
                
                <div class="flex gap-4 mb-6">
                    @if($sale->whatsapp_number ?? $sale->whatsapp)
                        <a href="https://wa.me/{{ $sale->whatsapp_number ?? $sale->whatsapp }}" target="_blank" class="w-10 h-10 rounded-full bg-green-500 text-white flex items-center justify-center hover:bg-green-600 transition">
                            <i class="fab fa-whatsapp text-xl"></i>
                        </a>
                    @endif
                    @if($sale->facebook_link ?? $sale->facebook)
                        <a href="{{ $sale->facebook_link ?? $sale->facebook }}" target="_blank" class="w-10 h-10 rounded-full bg-blue-600 text-white flex items-center justify-center hover:bg-blue-700 transition">
                            <i class="fab fa-facebook-f text-xl"></i>
                        </a>
                    @endif
                    @if($sale->instagram_link ?? $sale->instagram)
                        <a href="{{ $sale->instagram_link ?? $sale->instagram }}" target="_blank" class="w-10 h-10 rounded-full bg-pink-600 text-white flex items-center justify-center hover:bg-pink-700 transition">
                ...
                @if($sale->whatsapp_number ?? $sale->whatsapp)
                    <a href="https://wa.me/{{ $sale->whatsapp_number ?? $sale->whatsapp }}?text=Halo%20{{ urlencode($sale->name) }},%20saya%20ingin%20bertanya%20tentang%20truk%20HINO." class="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded-full text-center transition flex items-center justify-center gap-2">
            ...
            <div class="md:w-2/3 p-8">
                <h3 class="text-xl font-bold text-gray-900 mb-4 border-b pb-2">Tentang {{ $sale->name }}</h3>
                <div class="prose max-w-none text-gray-600 mb-8">
                    {!! nl2br(e($sale->bio ?? 'Berdedikasi untuk memberikan layanan terbaik dalam pemilihan unit truk HINO yang tepat untuk kebutuhan bisnis Anda.')) !!}
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden group">
                ...
                <div class="p-6">
                    ...
                    @if($sale->whatsapp)
                    <a href="https://wa.me/{{ $sale->whatsapp }}?text=Halo%20{{ urlencode($sale->name) }},%20saya%20tertarik%20dengan%20Hino%20300." class="text-green-600 font-bold hover:text-green-800 flex items-center gap-1">
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden group">
                ...
                <div class="p-6">
                    ...
                    @if($sale->whatsapp)
                    <a href="https://wa.me/{{ $sale->whatsapp }}?text=Halo%20{{ urlencode($sale->name) }},%20saya%20tertarik%20dengan%20Hino%20500." class="text-green-600 font-bold hover:text-green-800 flex items-center gap-1">
                        Tanya Detail <i class="fas fa-arrow-right text-sm"></i>
                    </a>
                    @endif
                </div>
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden group">
                ...
                <div class="p-6">
                    ...
                    @if($sale->whatsapp)
                    <a href="https://wa.me/{{ $sale->whatsapp }}?text=Halo%20{{ urlencode($sale->name) }},%20saya%20tertarik%20dengan%20Hino%20700." class="text-green-600 font-bold hover:text-green-800 flex items-center gap-1">
                        Tanya Detail <i class="fas fa-arrow-right text-sm"></i>
                    </a>
                    @endif
                </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col lg:flex-row gap-12 items-start">
                ...
                <div class="lg:w-1/2 w-full">
                    ...
                    @if($sale->documentation_photos && count($sale->documentation_photos) > 0)
                        <div class="grid grid-cols-2 gap-4">
                            @foreach($sale->documentation_photos as $doc)
                                <img src="{{ asset('storage/'.$doc) }}" class="w-full h-48 object-cover rounded-xl shadow-md hover:opacity-90 transition cursor-pointer">
                            @endforeach
                        </div>
                    @else
